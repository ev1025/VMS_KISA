...
let LB = { cat: null, clip: null, sec: 0, boxes: [], src: "none", mode: null };
  ...
  const afterShow = () => {
    Promise.all([samLabels(clip), autoLabels(clip), gtLabels(clip)]).then(([sam, dino, gt]) => {
      ...
      if (!hasHand(saved)) {
        ...
        const more = saved === null && LB.mode === "person";
...
const FIRE = { objs: [1, 2], name: { 1: "불", 2: "연기" }, color: { 1: "#f85149", 2: "#a371f7" }, cls: o => Math.max(0, Math.min(1, o - 1)), obj: c => (c === 1 ? 2 : 1) };
...
const PROP_MODES = [["separate", "분리 전파"], ["joint", "공동 전파"], ["detect", "DINO+SAM"]];   // 전파 방식(화재 실험용). 기본값은 서버가 정한다
let PROP_MODE = null; try { PROP_MODE = localStorage.getItem("kisa_prop_mode") || null; } catch (e) {}
...
function renderEditor(f) {
  ...
  const bFuse = mkBtn("DINO+SAM 감지", "이 프레임에서 Grounding DINO 로 불·연기 박스를 찾고 SAM 으로 마스크를 따서 참조샷으로 넣는다"); bFuse.hidden = !isFire();
  const selMode = el("select"); selMode.title = "전파 방식(화재 실험용)"; selMode.hidden = !isFire();
  selMode.style.cssText = "height:28px;background:var(--panel);color:var(--mut);border:1px solid var(--line);border-radius:6px;font-size:11px;padding:0 4px";
  PROP_MODES.forEach(([k, label]) => { const o = el("option"); o.value = k; o.textContent = label; selMode.appendChild(o); });
  selMode.value = PROP_MODE || (window.PROP_DEFAULT || "separate");
  selMode.onchange = () => { PROP_MODE = selMode.value; try { localStorage.setItem("kisa_prop_mode", PROP_MODE); } catch (e) {} };
  ...
  [bUndo, bRedo, bHand, bGT, bFuse, bGo, selMode, bClr, bRev, tstat, pstat, bReset].forEach(b => rowAct.appendChild(b));
  ...
  const draw = (drag, dcls) => {
    ...
    LB.boxes.forEach((b, i) => { const q = seedForBox(i); s += rectSvg(b[1] * f.W, b[2] * f.H, b[3] * f.W, b[4] * f.H, q ? samCol(q.obj) : col, LB.src === "dino"); });
    ...
    SM.seeds.filter(sd => near(sd.t, f.t) && !(sd.obj === SM.cur && SMASK)).forEach(sd => {   // 이 프레임 참조샷: 윤곽선 + 번호 + 점
      ...
      s += polySvg(sd.poly, f.W, f.H, cc, MASK_FILL()) + numAt(sd.box, cc, sd.obj);
      (sd.pts || []).forEach(p => { s += `<circle cx="${p[0] * f.W}" cy="${p[1] * f.H}" r="3" fill="${p[2] ? "#58a6ff" : "#f85149"}" stroke="${cc}" stroke-width="1"/>`; });
    });
    if (SMASK) { const cc = samCol(SM.cur); s += polySvg(SMASK.poly, f.W, f.H, cc, MASK_FILL()); if (SMASK.box) s += numAt(SMASK.box, cc, SM.cur); }
  ...
  const saveNow = async () => {                       // 화면 박스 → 손라벨 저장. DINO·정답 초안에서 손댄 프레임은 객체가 잡은 박스만 넘긴다(SAM 전파 결과는 보이는 그대로 손라벨로)
    if (LB.src === "dino" || LB.src === "gt") dropUnownedPrefill();
    try {
  ...
  async function applyMask(t, body, hit) {           // 서버 SAM 마스크 → 현재 객체의 박스·참조샷 → 손라벨 저장
    ...
    seedSet(r.box, r.poly, SP, idx); if (LB.src === "dino" || LB.src === "gt") dropUnownedPrefill();
  ...
  bHand.onclick = async () => {                      // 토글: 켜면 손라벨 → 참조샷(구간 안 전부), 다시 누르면 그때 넣은 참조샷만 제거
    if (SM.handRef) {
      ...
      SM.objs = isFire() ? objsFor() : SM.objs.filter(o => used.has(o) || o === 1);
    ...
    // 외형(색 히스토그램): 프레임 썸네일을 받아 박스 영역의 색 분포를 잰다 — 교차·무리 이동에서 위치만으로 못 가르는 신원을 가른다(사람 클립만)
    const hists = {};
    if (!isFire() && frames.length) {
    ...
    frames.forEach(t => {
      ...
      const taken = [], cand = [];
      boxes.forEach((b, i) => { if (taken.some(q => iou4(box4(q), box4(b)) > 0.7)) return; taken.push(b); cand.push({ b, i, cx: b[1] + b[3] / 2, cy: b[2] + b[4] / 2, hist: (hists[t] || [])[i] || null, obj: null }); });
      if (isFire()) cand.forEach(c => { c.obj = objOfCls(c.b[0]); });   // 화재: 클래스가 곧 객체(불=1 연기=2)
      else {
      ...
      cand.forEach(c => {
        ...
        SM.seeds.push({ t, obj, box: box4(b), poly: [], pts: [], i, fromHand: true });
      });
    });
  ...
  gtLabels(f.clip).then(gt => { const has = (gt && ((gt.points && Object.keys(gt.points).length) || (gt.frames && Object.keys(gt.frames).length))); if (mine()) { bGT.hidden = !has; updateTStat(); draw(); } });
  bGT.onclick = async () => {                        // 정답 → 참조샷
    ...
    for (const it of items) {
      ...
      if (!box && it.pt) {                             // 점 → SAM 마스크 → 박스
        const r = await postJSON("/api/sam2_mask", { clip: f.clip, t: it.t, pts: [[it.pt[0], it.pt[1], 1]] }).catch(() => ({}));
        if (r.box) { box = r.box; poly = r.poly || []; pts = [[it.pt[0], it.pt[1], 1]]; }
      }
      if (!mine()) return;                             // 그사이 다른 클립으로 갔다
      n++; pstat.innerHTML = spin(Math.round(n / items.length * 100));
      if (!box) continue;
      if (!SM.objs.includes(it.obj)) SM.objs.push(it.obj);
      SM.seeds = SM.seeds.filter(q => !(near(q.t, it.t) && q.obj === it.obj));
      SM.seeds.push({ t: it.t, obj: it.obj, box, poly, pts, i: null, fromGT: true });
      if (near(it.t, f.t)) { LB.boxes.push([samCls(it.obj), box[0], box[1], box[2], box[3]]); LB.src = "hand"; }
    }
    SM.objs.sort((a, b) => a - b); SM.seeds.sort((a, b) => a.t - b.t || a.obj - b.obj);
    if (!SM.objs.includes(SM.cur)) SM.cur = SM.objs[0];
  ...
  bFuse.onclick = async () => {                      // 화재: Grounding DINO 박스 → SAM 마스크 → 불(1)·연기(2) 참조샷(이 프레임)
    ...
    if (!got.length) { flash('<span style="color:var(--mut)">감지 없음</span>', 2500); return; }
    snap();
    got.forEach(([o, v]) => {
      const obj = +o;
      SM.seeds = SM.seeds.filter(q => !(near(q.t, t) && q.obj === obj));
  ...
  bReset.onclick = async () => {
    ...
    try {
      ...
      SM.seeds = []; SM.objs = objsFor(); SM.cur = 1; SM.handRef = false; SM.a = null; SM.b = null; SP = []; SMASK = null;
      hist.length = 0; redo.length = 0; _PENDING = null;   // 이력도 비운다(배열을 그대로 두고 비워야 Ctrl+Z 가 옛 배열을 안 본다)
      LB.boxes = []; LB.src = "none"; f.saved = null;
      styleHand(); drawObjs(); await refreshSam(); if (typeof updateRawBadge === "function") updateRawBadge(f.stem);
  ...
  bGo.onclick = async () => {
    ...
    const start = await postJSON("/api/sam2_propagate_start", { clip: f.clip, seeds, a, b, step: _step(), mode: isFire() ? selMode.value : "separate" }).catch(() => ({ err: "요청 실패" }));
  ...
  document.onkeydown = ev => {
    ...
    if (/^[1-8]$/.test(ev.key)) {                                                        // 숫자 = 객체 선택(화재는 1 불 · 2 연기 만)
      const n = +ev.key;
  ...
  ED = {
    clip: f.clip, _tok: MY, loadSam, fillShots, watch: () => watchJob(),
...
const boxCol = (d, b) => d.src === "hand" ? SRC_COLOR.hand : (isFire() ? samCol(objOfCls(b[0])) : SRC_COLOR.sam);
...
async function postLabel(clip, t, W, H, boxes, src, clear) {
  ...
  const res = await postJSON("/api/savelabel", { clip, t, src, file: `${clip}_${String(t).padStart(4, "0")}.png`, W, H, boxes, kind, clear: !!clear });
...
function renderShotRow(row, f, hooks) {
  ...
  const center = () => { const nr = [...row.children].sort((a, b) => Math.abs(+a.dataset.t - f.t) - Math.abs(+b.dataset.t - f.t))[0]; if (nr) requestAnimationFrame(() => { row.scrollLeft = nr.offsetLeft - row.offsetLeft - row.clientWidth / 2 + nr.offsetWidth / 2; }); };
