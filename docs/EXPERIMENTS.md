...
### 5.1 채점 규칙 두 벌 (구 규칙 vs 신규칙) — 2026-09-09 부터 둘 다 기록
- **구 규칙**(`score_kisa.py` 4개: 기본/fire만/결합/결합+타일가정) = 보수적. fresh 큐의 57.14 등은 이것
- **신규칙**(`fire_rule2.py` 로직을 score_kisa 에 내장): 불 임계를 짧은 창에서 **연속 hits 회** + 연기는 **앞 60초 기준선 대비 상승량**. (fth, win/hits, 연기Δ) 540조합 스윕 → 상위 5개를 `신규칙 …` 줄로 출력, 시계열은 `results/par/tl/<tag>.json`
- 2026-09-07 방화 **88.89**(human_full = dataset_24k+손라벨×5) 는 신규칙 점수. 같은 모델 구 규칙은 70.59 → 점수 도약의 대부분은 **규칙 재설계**. 미검 2편(C00_195/216)은 연기 conf 가 영상 내내 0.92 고정이라 상승량이 없음 = 모델 한계
- 신규칙은 10편에 맞춘 스윕이라 **과적합 위험**. 결과탭은 `F1 최고`(대개 신) 와 `F1 구규칙` 두 열로 나란히 보여준다. 최종 판단은 두 값이 같이 오르는 실험을 고른다
- 2026-09-10 실증: 신규칙 **고정 프리셋**(human_full 최적 f0.50 5/12 연기+0.2)을 fire_snowfull 에 그대로 적용하면 **58.82**(구 규칙 82.35). 신규칙 파라미터는 모델별 스윕값 → 다른 모델로 전이 안 됨. 배포 SA 생성기(VMS tools/kisa_items.py)의 방화 기본 규칙은 결합 규칙, 신규칙은 제출 모델의 시계열(results/par/tl/<tag>.json)로 재스윕한 값만 쓴다.

## 다음 실험 후보 (2026-09-11 정리, 라이선스 문제 데이터 제외: LLVIP·KAIST·FLIR 사용 안 함)
채점셋 mAP(배포 검증영상 라벨, 학습 미포함)가 생겨 실험을 정직하게 비교할 수 있게 됐다. 현재 병목 = 리콜(P 0.6 · R 0.22~0.33), 특히 연기(mAP50 0.15).

### 방화
| 우선 | 실험 | 근거 | 데이터 |
|---|---|---|---|
| 1 | **우리 라벨·하드네거 오버샘플**(`fire_azim_hn_s`, 큐 진행) | 채점 장면 배경을 배우면 오탐↓, 손라벨 ×4 로 리콜↑ | 48k + fasdd + azimjaan + handset_fire_hn ×4 |
| 1 | 연기 손라벨 보강 후 재학습 | 연기 mAP50 0.15 = 불의 절반. 라벨 수 자체가 부족 | 연구개발 방화 클립 연기 드래그 라벨 |
| 2 | 하드네거 비율 스윕(`--neg 3/5/8`) | 배경 비율이 리콜에 미치는 영향 확정 | 같은 조합 |
| 2 | imgsz 960(11s) | 초기 작은 불·먼 연기. 채점 영상이 1280×720 | 같은 조합 |
| 2 | 11m·11l 완주 비교 | 11m 이 중간 에폭에도 mAP50 0.30(최고). 배포 NPU 예산 안이면 11m 검토 | m 진행 중 |
| 3 | 연기 클래스 가중·오버샘플(연기 있는 프레임 ×3) | 클래스 불균형 | fasdd 연기 우세 프레임 |
| 3 | 2단계 학습(coarse 대규모 → fine KISA 장면) | 도메인 갭(채점셋 mAP50 0.25 vs 학습 검증 0.87) | 1단계 기존 큐 best → 2단계 handset_fire_hn 만 저 LR |
| 3 | KD(11x 티처 → 11s) | 경량 예산 안 정확도 | 티처 fresh_snf_x |
| 4 | 시간 축(연속 프레임 채널 스택 또는 후처리 링버퍼 튠) | 채점 규칙이 시계열. 모델보다 규칙 스윕 재조정 | par/tl 시계열 |

### 사람
| 우선 | 실험 | 근거 | 데이터 |
|---|---|---|---|
| 1 | **KISA 라벨만 미세조정**(`person_kisa_only_s`, 큐) | 사전학습 가중치가 이미 person 을 알고 있다. KISA 장면 22클립(손 144·전파 936)만 얹는 게 가장 싸다 | handset_person |
| 1 | **COCO person + KISA ×5**(`person_coco_kisa_s`, 큐) | COCO 로 일반성 유지 + KISA 오버샘플로 장면 적응 | trainset_person_20260911 |
| 2 | imgsz 960(`person_coco_kisa_s960`, 큐) | 채점 영상 사람이 작다(먼 카메라). 앞선 침입 미검 2편이 탐지 실패 | 같은 셋 |
| 2 | 11m(`person_coco_kisa_m`, 큐) | 용량 여유 확인 | 같은 셋 |
| 3 | AI-Hub 71953 다각도 키프레임 박스 → 프레임 추출해 추가 | 받아둔 Validation 배회·침입 404편, 라벨 `evidence.obj_bbox`(키프레임). 단일 객체라 규칙 검증엔 못 쓰지만 검출 학습엔 가능 | aihub71953_다각도 |
| 3 | aihub_침입쓰러짐 손라벨 확대 | 80편 중 라벨 적음(78박스) | 대시보드 전파 |
| 4 | 야간·IR: 라이선스 있는 출처 없음 → KISA 야간 클립 손라벨·전파로만 | LLVIP·KAIST·FLIR 제외 | 연구개발 사람영상 야간 139편 |

### 공통
- 모든 실험은 채점셋 mAP(eval_map.json)와 F1(방화) 두 값을 남긴다. 비교는 채점셋 mAP 기준, 제출 후보는 F1 과 함께.
- 하드 네거티브 개수(불연기 5→3, 사람 상한 손+전파)는 스윕으로 정한다.

## 5. 그리드 서치 계획 (2026-09-11 수립, 채점셋 mAP 기준)

### 왜 다시 짜는가
- 옛 검증 mAP 는 학습 목록에서 뽑은 600장(누수)이었다. 이제 **채점셋 mAP**(배포 검증영상 771프레임, 학습 미포함)가 있어 요인 비교가 정직해졌다. 채점 F1 은 10편이라 값이 몇 단계로만 떨어져(66.67·82.35·88.89·94.74) 요인 스크리닝에는 둔하다.
- 지금 결과(2026-09-11): 채점셋 mAP50 azimjaan 0.309 · 11m(중간) 0.303 · wildall 0.247 · full 0.221 · wildpos 0.220 · snowfull 0.230 · 24k_base 0.127. 연기 mAP50 은 0.15~0.23 으로 불(0.29~0.39)의 절반, 리콜 0.22~0.33 이 병목.
- 옛 결론 중 살릴 것: 손라벨 배수는 중간(×5~×10)이 최적(D), 베이스는 25%에서 포화(E), 범용 배경만 넣으면 오검↔미검 교환(G). 단 전부 옛 F1 기준이라 mAP 로 재확인 대상.

### 비용
- 잡당 에폭 ≈ 6~14분(2잡 동시, 960 사본). 80에폭 ≈ 8~18시간. 하루에 실험 2~3개가 한계 → 요인을 **한 번에 하나씩**, 스크리닝은 **40에폭**으로.
- 1단계 14잡 × 40에폭 ≈ 4~5시간/잡 → 2잡 동시 ≈ 35시간(1.5일). 2단계 8잡 × 80에폭 ≈ 4일.

### 1단계 스크리닝 (`configs/queue_grid_fire_20260912.yaml`, yolo11s · 640 · 40에폭)
기준 B = 48k + fasdd_snowfog + fasdd + azimjaan. 손라벨 = handset_fire_hn(손 412 · 전파 56 · 하드네거 N).

| 그리드 | 요인 | 값 | 잡 | 판단 |
|---|---|---|---|---|
| G0 | 기준선 | B | 1 | 40에폭 기준값 |
| G1 | **하드 네거티브** | 없음(hn0) / 우리 3 / 우리 5 / 우리 8 / 우리 5+범용 안개(wildfire_fog_neg) | 5 | 연기 mAP50·P 가 오르고 R 이 안 떨어지는 값 |
| G2 | **손라벨 오버샘플** | ×0(G0) / ×2 / ×4(G1-hn5) / ×8 | 2 | 불·연기 mAP50 최대, 과적합(에폭 mAP 급락) 없는 값 |
| G3 | **라벨 있는 원천셋** | B−azimjaan / B−fasdd / B+dfire / B+wildfire_pos | 4 | 셋마다 기여도. 빼도 안 떨어지면 제외(학습 시간 절감) |
| G4 | **베이스 비율(손:외부)** | 48k 25% / 50% / 100%(G1-hn5) | 2 | 외부 대규모가 얼마나 필요한지. 포화점 확인 |

- 동률 기준: 같은 요인 안 mAP50 차이 0.02 미만 = 차이 없음(771프레임 잡음). 그 안이면 **데이터 적은 쪽·학습 빠른 쪽**을 택한다.
- 각 잡은 채점셋 mAP(eval_map.json)·F1(score.txt) 둘 다 자동 기록. 결과 탭에서 비교.
- 손라벨 배수·비율은 **손라벨이 늘어나면 다시 스윕**해야 한다(지금 703장 기준). 연기 라벨이 늘면 G2 재실행.

### 2단계 확정 (80에폭)
- 1단계 상위 2조합 × 모델 {11s, 11m} × 해상도 {640, 960} = 8잡. 배포 NPU 예산에 따라 11m 을 뺄 수 있다.
- 최종 선택: 채점셋 mAP50 과 F1 이 **같이** 높은 것. F1 만 높은 건 규칙 스윕 과적합 의심.

### 3단계 (손라벨 증가 후)
- 연기 손라벨 보강 뒤: G2 재스윕 + 하드네거 재스윕. 손라벨 50%/100% 학습 곡선(라벨을 더 만들 가치가 있는지).
- 2단계 학습(coarse 외부 → fine 우리 라벨만 저 LR), 11x→11s KD.

### 사람 (`queue_mix_20260911.yaml`, 방화 큐 뒤 자동 시작)
- kisa_only(우리 라벨 1,191장) / coco+kisa×5 / coco+kisa×5@960 / 11m. 지표 = evalset_person mAP(1,289프레임). 이후 오버샘플 ×2/×10·하드네거 유무 그리드.

### 지금 큐(방화 13잡) 정리 제안
- 남은 것 중 그리드와 겹치거나(dfire·zoom·ext3·sink) 채점셋 mAP 없이 짠 조합은 빼고, 모델 크기 비교(11n·26n·11l·11x)만 남기면 1단계 그리드를 하루 앞당길 수 있다. (사용자 결정)
