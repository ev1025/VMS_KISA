...
| 저장소 | 파일 | 만드는 곳 | 뜻 |
|---|---|---|---|
| 손라벨 | `data/학습데이터/손라벨/{person,fire,image}_labels.json` | 편집기 저장(탭·드래그·검수 수정) | 학습 데이터. 행에 `obj`(객체 번호, 2026-09-11) · `cls -1` = 검토완료(박스 없음) 마커 · `eval:true` = 채점 클립(학습 제외) |
...
- 검토완료 마커(`cls -1`)는 정답 초안만 막는다. SAM 전파 결과는 그 위에 보인다.
...
- `LB` 화면 상태(mode·clip·sec·boxes·src). `src` 는 hand/sam/gt/none.
- `samState(clip)` = `SM` (참조샷 seeds·objs·cur·구간 a/b·propFrames). 브라우저 localStorage 에 남는다.
- 캐시: `SAMMAP`/`SAMPOLY`/`SAMSEEDS`(전파 저장소), `GTMAP`, `SAMFR`(목록 배지). 무효화는 `samInvalidate`/`samForget` 만 쓴다.
...
> DINO(Grounding DINO) 초안·자동 감지·detect 전파는 2026-09-11 걷어냈다. 의사라벨을 학습에 쓰지 않기로 해서 남길 이유가 없었다. 배치 결과(자동라벨/dino)도 삭제.

## 편집 규칙 (2026-09-11 확정)
- **객체 번호는 박스의 고정 속성**(6번째 값, 손라벨 행 `obj`). 옮기기·크기조절·삭제는 그 번호만 따르고 거리(IoU)로 객체를 추정하지 않는다. 번호 없는 옛 박스는 프레임을 열 때 한 번만 번호를 박는다(`pinObjs`).
- 전파 박스를 옮기면 손라벨이 된다(재전파가 덮지 않게). 객체 번호는 유지된다. 옮기기는 참조샷을 만들지 않는다.
- 참조샷 만들기 = 탭(SAM 점) · 빈 곳 드래그(새 박스) · C(이전 프레임 복사) · 손라벨 참조. 연기(화재 객체 2)는 참조샷을 만들지 않는다.
- 삭제: 프레임 삭제(미리보기 × · 검수) = 그 프레임 객체 전부 + 참조샷. 객체 칩 × = 그 프레임에서 그 객체만, 다른 객체 남으면 프레임 유지·없으면 프레임 삭제. Del = 박스 + 그 참조샷.
- 전파 버튼: `전파` = 시작~종료(비면 클립 전체, 참조샷이 밖이면 그것까지). `이어서 전파` = 결과가 있고 지금 프레임에 참조샷이 있을 때, 범위 안에서 지금 프레임~종료만 뒤로. 결과 있는 클립에서 전체 전파는 구간·참조샷을 보여주는 확인창. 전파 뒤 참조샷은 유지되고 `전파 지우기`도 참조샷은 남긴다.
- 제외점만으로는 SAM 을 부르지 않는다. 화재에서 다른 클래스 박스를 탭하면 그 박스의 객체로 바뀐다(연기 박스는 선택만).
- DINO 초안·자동 감지·detect 전파는 2026-09-11 제거.
