#!/bin/bash
# 리콜(미검) 개선용 실험: 큰 imgsz + 용량. queue2(fog) 끝나면 실행.
V=/NHNHOME/WORKSPACE/26mss002_E3/vms; PY=$V/.venv/bin/python
BAE="$V/data/원본데이터/kisa_배포_검증영상/deploy_val/방화(10개)/배포"
log(){ echo "[$(date +%m-%d\ %H:%M)] $*"; }
log "queue2(fog) 종료 대기"
while tmux has-session -t queue2 2>/dev/null; do sleep 120; done
# 인자: NAME MODEL IMGSZ BATCH [추가데이터...]
run_exp(){
  local NAME=$1 MODEL=$2 IMG=$3 BAT=$4; shift 4; local EXTRA=("$@")
  ...
  printf "path: %s\ntrain: images/train\nval: images/val\nnc: 2\nnames: ['fire','smoke']\n" "$DS" > "$DS/data.yaml"
  log "$NAME 학습($MODEL img=$IMG bat=$BAT, $(ls $DS/images/train|wc -l)장, +[${EXTRA[*]}])"
  cd $V; CUDA_VISIBLE_DEVICES=0 $PY model.py train --models $MODEL --data "$DS/data.yaml" --project $V/runs --device 0 --batch $BAT --epochs 80 --imgsz $IMG --multi-scale --no-export --force >/dev/null 2>&1
  mv $V/runs/$MODEL $V/runs/$NAME 2>/dev/null
  log "$NAME 채점(img=$IMG)"
  { echo "=== $NAME 타일 ==="; $PY $V/score_kisa.py $V/runs/$NAME/weights/best.pt --videos "$BAE" --gt "$BAE" --stride 0.5 --imgsz $IMG --tiles --tag "$NAME"; } > $V/results/$NAME.txt 2>&1
  rm -rf "$DS"; log "$NAME 완료"
}
T=$(date +%Y%m%d)
run_exp fire_s960_$T   yolo11s 960  128
run_exp fire_s1280_$T  yolo11s 1280 64
run_exp fire_m960_$T   yolo11m 960  96  fasdd_yolo
log "QUEUE3(recall) DONE"
